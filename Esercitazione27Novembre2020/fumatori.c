 ...
 * I threads fumatori eseguono in un ciclo senza fine, mentre il thread tabaccaio esegue NTIMES iterazioni.
 * Il tabaccaio usa una funzione random, per stabilire quali dei due 'ingredienti' deve mettere sul tavolo: tale funzione richiede l'inizializzazione del seme. */
#include <pthread.h>
#include <semaphore.h>
#include <stdio.h>
#include <stdlib.h>
...
#include <time.h>
#define NTIMES 10
#define NUM_THREADS 4	/* numero di threads: 1 tabaccaio e 3 fumatori */
...
void *eseguiFumatore(void *id)
{
   ...
   for (i = 0; ; i++) /* while (true)  il fumatore DEVE essere un ciclo senza fine */
   {
	...
	/* si deve segnalare al tabaccaio che sono stati presi i due 'ingredienti' dal tavolo */
 	sem_post(&SEM[0]);
   	/* fine sezione critica */
   	pthread_mutex_unlock(&MUTEX);
...
int main (int argc, char **argv)
{
   ...
   for (i=0; i < NUM_THREADS; i++)
   {
   	...
   	{
        	printf("Problemi con l'inizializzazione del semaforo %d-iesimo\n", i);
        	exit(5);
   	}
   }
