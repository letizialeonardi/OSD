/* OBIETTIVO: generare un numero non noto di threads che eseguono la modifica di una variabile globale in un ciclo, all'interno del quale stampano in valore corrente di
 tale variebile, il loro numero d'ordine e il loro identificatore; il risultato voluto sarebbe quello che ogni thread dovrebbe eseguire il ciclo di modifica in mutua esclusione ==> poiche' il ciclo di modifica è considerato una sezione critica, il risultato della esecuzione e' quello sperato */
#include <pthread.h>
#include <stdio.h>
#include <stdlib.h>
...
NDIVISA fra tutti i thread) per 10 volte: N.B. invece che il valore costante 10
potrebbe essere passato un altro parametro all'atto della invocazione */
